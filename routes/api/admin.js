...
router.post('/log', checkLogin, function(req, res, next) {
	...
	pool.getConnection(function(err, connection) {
		...
		connection.query(log.countAllById, [param.id], function(err, result) {
			if (result) {
				total = result[0]['count(*)'];
				if (total > 0) {
					connection.query(log.queryAllById, [param.id, page, pageSize], function(err, result) {
						if (result) {
							result.forEach((item) => {
