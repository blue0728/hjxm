<template>
<div>
	<div><el-button type="primary" icon="plus" @click="go({name: 'ad-add'})">添加轮播</el-button></div>
	<br>
        ...
        <el-table-column prop="time" width="200" label="更新时间"></el-table-column>
      	...
      	<el-table-column width="250" label="操作">
	      	<template scope="scope">
		        ...
		        <el-button size="small" @click="go({name: 'ad-detail', params: {id: scope.row.id}})">编辑</el-button>
...
export default {
	...
	methods: {
		dele: function(id){
			this.$confirm('确定删除轮播吗？删除后不可恢复！！！', '提示', {
	         	confirmButtonText: '确定',
	          	cancelButtonText: '取消',
	          	type: 'warning'
	        }).then(() => {
	          	this.$http.post('/api/adver/dele',{
	          		id: id
	          	}).then(function(res){
		...
		setStatus: function(id, status){
			this.$http.post('/api/adver/set-status',{
				status: status,
				id: id
			}).then(function(res){
				if(res.data.status == 'success'){
